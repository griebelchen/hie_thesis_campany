...
pve_tab3 <- read.csv("data/pve_table2.csv")
...
<<results=verbatim, echo=FALSE,message=FALSE, warning=FALSE>>=
  datatab1 <- xtable(pve_tab1, label="table:Table1")

  print(datatab1, include.rownames=FALSE,sanitize.colnames.function=function(x){x},
        floating.environment='sidewaystable', caption.placement="top",
         hline.after = getOption("xtable.hline.after", c(-1,0,7,nrow(datatab1))))
