...
\\
\\
\textbf{Figure 3.3}. The response of A\textsubscript{n} to g\textsubscript{s} (a) and g\textsubscript{m} (b) for sun leaves measured at high light and shade leaves measured at both low and high light under their respective elevated and ambient temperature treatments. Lines represent either smoothed regressions from a generalized additive model fit (a) or linear model fits (b). Grey areas are 95\% confidence intervals for the mean.
\\
\\
...
\\
\\
{\textdelta}\textsuperscript{13}C  Carbon isotope ratios expressed relative to standard Vienna Pee Dee Belemnite
\\
\\
...
library(xtable)
pve_tab1 <- read.csv("data/pve_table1.csv")

colnames(pve_tab1) <- c("Volume (L)",
                        "Seedling~mass~(g)",
...
pve_tab2 <- read.csv("data/root_table.csv")
colnames(pve_tab2) <- c("Root~Nitrogen~(\\%)",
                        "SRL~(cm~m\\textsuperscript{-1})",
                        "FRLD~(m~dm\\textsuperscript{-3})")


pve_tab3 <- read.csv("data/phystable.csv")
colnames(pve_tab3) <- c("Volume~(L)",
                        "\\textit{A}\\textsubscript{max}~({\\textmugreek}mol~m\\textsuperscript{-2}~s\\textsuperscript{-1})",
                        "\\textit{R}\\textsubscript{dark}~({\\textmugreek}mol~m\\textsuperscript{-2}~s\\textsuperscript{-1})",
                        "\\textit{J}\\textsubscript{max}",
                        "\\textit{Vc}\\textsubscript{max}",
                        "\\textit{g}\\textsubscript{s}~(mol~m\\textsuperscript{-1}~s\\textsuperscript{-1})",
                        "\\textit{g}\\textsubscript{1}")


pve_modtab<- read.csv("data/pve_modeltable.csv")
pve_modtab[1] <- c("Leaf area\\textsubscript{i}", "Leaf mass\\textsubscript{i}","Stem mass\\textsubscript{i}",
                   "Root mass\\textsubscript{i}", "\\epsilon\\textsubscript{c}", 
                   "R\\textsubscript{coarse}~\\textsubscript{root}","R\\textsubscript{fine}~\\textsubscript{root}",
                   "R\\textsubscript{stem}", "C\\textsubscript{day}","\\Lambda") 
...
%pvetable1
 <<results='asis'>>=
 pvetab1 <- xtable(pve_tab1, label="table:Table 2.1",caption="Responses of plant and leaf characteristics of \\textit{Eucalyptus tereticornis} seedlings to soil volume treatments. Each value reflects the mean($\\pm$1 standard error) for each treatment. Seedling mass and leaf {\\textdelta}\\textsuperscript{13}C values are from final harvest. Values of leaf starch, sugars, nitrogen and SLA represent overall means across measurement campaigns (n=6). Different letters represent significant differences between treatments. The volume effect P value represents the overall difference between seedlings with soil volume restriction and the control seedlings.")

 print(pvetab1, include.rownames=FALSE,sanitize.colnames.function=function(x){x},
       floating.environment='sidewaystable',caption.placement="top",
        hline.after = getOption("xtable.hline.after", c(-1,0,7,nrow(pvetab1))))
@
...
%modtable
 <<results='asis'>>=
 modtab <- xtable(pve_modtab, label="table:Table 2.S1",caption="Seedling growth model default parameters.")

 print(modtab, include.rownames=FALSE,sanitize.colnames.function=function(x){x},
       floating.environment='sidewaystable',caption.placement="top",
        hline.after = getOption("xtable.hline.after", c(-1,0,7,nrow(modtab))))
 @
...
resourcetab <- read.csv("data/wtc3_resource_table.csv", stringsAsFactors = FALSE)
pval <- c(0.785, 0.001, 0.028, 0.002, 0.001, 0.3486, 0.6385, 0.973)
resourcetab2 <- rbind(resourcetab, c("P value","", as.character(pval)))
colnames(resourcetab2) <- c("Leaf", "Temperature", "LMA", "N\\textsubscript{a}", "Vc\\textsubscript{max}", 
          "J\\textsubscript{max}","K","WP\\textsubscript{pre}","WP\\textsubscript{mid}", 
          "{\\textdelta}\\textsuperscript{13}C")


getab <- read.csv("data/wtc3_ge_table.csv", stringsAsFactors = FALSE)
pval2 <- c(0.001,	0.001,	0.001,	0.001,	0.001,	0.005,	0.001,	0.001)
getab2 <- rbind(getab, c("P value","","", as.character(pval2)))
colnames(getab2) <- c("Leaf", "Light","Temperature", "A", "g\\textsubscript{s}", "g\\textsubscript{m}", "ITE","E","VPD","C\\textsubscript{i}", "C\\textsubscript{c}")
...
%wtc1tab
 <<results='asis'>>=
 wtc3tab1 <- xtable(resourcetab2, label="table:Table 3.1",caption="\\textit{Eucalyptus tereticornis} leaf morphological and physiological traits between full sun and shade leaves under ambient and elevated temperature treatments. Leaf mass per area, N\textsubscript{a}, {\\textdelta}\\textsuperscript{13}C, $\\Psi\\textsubscript{pd}$, $\\Psi\\textsubscript{l}$ and K\textsubscript{l} values represent treatment mean ($\\pm$~1~standard error) across measurement campaigns (n=6). Values of Vc\\textsubscript{max} and J\\textsubscript{max} are treatment mean ($\\pm$ 1 standard error) from AC\\textsubscript{i} curves measured in each chamber at saturating light. Units of LMA and Leaf N\\textsubscript{area} are g~m\\textsuperscript{2}, K\\textsubscript{l} is mmol~m\\textsuperscript{-2}~s\\textsuperscript{-1} MPa, WP is MPA and {\\textdelta}\\textsuperscript{13}C is \\permil. Different letters represent significant differences between leaf type and temperature treatments. The P value represents the overall effect between each unique combination of leaf type and temperature treatment for each trait.")

 print(wtc3tab1, include.rownames=FALSE,sanitize.colnames.function=function(x){x},
       floating.environment='sidewaystable',caption.placement="top",
        hline.after = getOption("xtable.hline.after", c(-1,0,4,nrow(wtc3tab1))))
 @
...
%wtc1tab
 <<results='asis'>>=
 wtc3tab2 <- xtable(getab2, label="table:Table 3.2",caption="\\textit{Eucalyptus tereticornis} leaf gas exchange parameters for sun and shade leaves under ambient and elevated temperature treatments. Each value reflects the mean ($\\pm$~1~standard error) for each treatment across gas exchange campaigns (n=6). Units for A\\textsubscript{n} and E are {\\textmugreek}mol~m\\textsuperscript{-2}~s\\textsuperscript{-1}, for g\\textsubscript{s} and g\\textsubscript{m} are mol~m\\textsuperscript{-2}~s\\textsuperscript{-1} and for VPD are kPa. Different letters represent significant differences between leaf type, light environment and temperature treatments. The P value represents the overall effect between each unique combination of leaf type, light environment and temperature treatment for each parameter.")

 print(wtc3tab2, include.rownames=FALSE,sanitize.colnames.function=function(x){x},
       floating.environment='sidewaystable',caption.placement="top",
        hline.after = getOption("xtable.hline.after", c(-1,0,6,nrow(wtc3tab2))))
 @
...
\begin{figure}[h!]
    ...
    \caption{The response of A\textsubscript{n} to g\textsubscript{s} (a) and g\textsubscript{m} (b) for sun leaves measured at high light and shade leaves measured at both low and high light under their respective elevated and ambient temperature treatments. Lines represent either smoothed regressions from a generalized additive model fit (a) or linear model fits (b). Grey areas are 95\% confidence intervals for the mean.}
...
\begin{figure}[h!]
    ...
    \caption{Relationship between the observed discrimination of \textsuperscript{13}CO\textsubscript{2} measured during photosynthesis (\Delta\textsubscript{0})and measured C\textsubscript{i}/C\textsubscript{a} for sun leaves measured at high light and shade leaves measured at both low and high light. The solid line represents the theoretical line for C3 plants from \citet{evans1986carbon}.}
...
\begin{figure}[h!]
    ...
    \caption{The mean $\pm$~1 standard error of (a) intercellular CO\textsubscript{2} concentration (C\textsubscript{i}), (b) CO\textsubscript{2} concentration in the chloroplasts (C\textsubscript{c}) and (c) CO\textsubscript{2} drawdown from substomatal cavities to sites of carboxylation of sun leaves and shade leaves at both low and high light (C\textsubscript{i}-C\textsubscript{c}).}
...
\begin{figure}[h!]
    ...
    \caption{Daily maximum and minimum temperature (a), daily maximum VPD (b) and total daily PPFD (c) for each chamber across the experiment duration.}
...
\begin{figure}[h!]
    ...
    \caption{Response of A\textsubscript{n} (a), g\textsubscript{m} (b) and C\textsubscript{i}-C\textsubscript{c} to leaf temperature for sun leaves and shade leaves at low and high light. Shaded symbols represents each monthly measurement campaign. Solid lines, colored by leaf and light type, are fitted line for the relationship with each parameter and leaf temperature across all measurement campaigns. All parameters with no relationship are fitted with zero slope and the overall mean value for each treatment combination. Weak negative relationships with g\textsubscript{m} and increasing leaf temperature were detected with sun and shade leaves under their local light environment (R\textsuperscript{2}~=~0.16 and 0.08, respectively). }
...
<<echo=FALSE, message=FALSE, warning=FALSE>>=
wtc1_table <- read.csv("data/wtc1_data_table.csv", stringsAsFactors = FALSE)

Pinter <- read.csv("data/wtc1_p_sigs/P_interactions.csv")
Pinter <- round(Pinter, 3)

Pco2 <- read.csv("data/wtc1_p_sigs/P_co2.csv")
Pco2 <- round(Pco2, 3)

Ph2o <- read.csv("data/wtc1_p_sigs/P_h20.csv")
Ph2o <- round(Ph2o, 3)

###sla P values
Pinter_sla <- read.csv("data/wtc1_p_sigs/sla/P_interactions_sla.csv")
Pinter_sla <- round(Pinter_sla, 3)

Pco2_sla <- read.csv("data/wtc1_p_sigs/sla/P_co2_sla.csv")
Pco2_sla <- round(Pco2_sla, 3)

Ph2o_sla <- read.csv("data/wtc1_p_sigs/sla/P_h20_sla.csv")
...
wtc1_table2 <- rbind(wtc1_table, c("CO~2~ effect (P)",  Pco2_2$x))
wtc1_table3 <- rbind(wtc1_table2, c("Drought effect (P)",  Ph2o_2$x))
wtc1_table4 <- rbind(wtc1_table3, c("CO~2~ * Drought (P)",  Pinter_2$x))
...
%wtc1tab
 <<results='asis'>>=
 wtc1tab <- xtable(wtc1_table4, label="table:Table 4.1",caption="Final harvest C mass of above and belowground tissues, cumulative aboveground tree C uptake (F\\textsubscript{c,T}) and specific leaf area (SLA). Each value represents the mean ($\\pm$~1 standard error) for each treatment combination. Units for C mass and F\\textsubscript{c,T} are g~C, while SLA are cm\\textsuperscript{2}~g\\textsuperscript{-1}. For each variable, different letters represent significant differences between treatments from the overall model which includes CO\\textsubscript{s} * drought interactions. P values represent overall differences of CO\\textsubscript{s} or drought main effects and the CO\\textsubscript{s} * drought interaction.")

 print(wtc1tab, include.rownames=FALSE,sanitize.colnames.function=function(x){x},
       floating.environment='sidewaystable',caption.placement="top",
        hline.after = getOption("xtable.hline.after", c(-1,0,4,nrow(wtc1tab))))
 @
